# 모집단에서 추출한 표본의 개수(n)가 증가할수록, 표본평균의 분포가 정규분포와 닮아감
# 중심극한정리는 이러한 성질을 수리적으로 나타낸 것(식) 
### 모집단이 정규분포일 경우
options(digits = 4)
set.seed(9)
n = 10000
...
#### b2, b4, b32, b64 그래프로 확인하기
# 표본 크기가 증가할 수록 정규분포와 유사해짐
# 아래 작성 후 실행할 때 초기화하고 처음부터 실행하기!!
par(mfrow=c(2,2))
